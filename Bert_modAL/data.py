    ...
    def load(cls, root_dir: str, cache_dir: str, dataset: Dataset, handler, tokenizer: BertTokenizer, stride: int = 128, force_preprocess: bool = False, force_post_check: bool = False):
        ...
        if not force_preprocess:
            ...
            if os.path.exists(cache_filepath):
                ...
                if datetime.strptime(json.load(open('/Users/maxkeller/Documents/Uni/Softwaretechnik/Projektarbeit/ActiveLearningProject/Bert_modAL/version', 'r'))['preprocessed_data'], '%d/%m/%y %H:%M:%S') > datetime.fromtimestamp(os.path.getmtime(cache_filepath)):
